...
class ViewPembalap implements KontrakView{
    ...
    public function tampilPembalap($listPembalap): string {
        ...
        foreach($listPembalap as $pembalap){
            ...
            $tbody .= '<td class="col-actions">
                    <a href="index.php?entity=pembalap&screen=edit&id='. $pembalap->getId() .'" class="btn btn-edit">Edit</a>
                    <button data-id="'. $pembalap->getId() .'" data-entity="pembalap" class="btn btn-delete">Hapus</button>
                  </td>';
            $tbody .= '</tr>';
            $no++;
        }
        ...
        if (file_exists($templatePath)) {
            ...
            // replace the Total placeholder block (look for the small "Total:" string and append count)
            $template = str_replace('<div style="color:var(--muted);font-size:13px">Total:</div>', '<div style="color:var(--muted);font-size:13px">Total: ' . $total . '</div>', $template);
            return $template;
        }
    ...
    public function tampilFormPembalap($data = null): string {
        ...
        if ($data) {
            ...
            // set entity
            $template = str_replace('<input type="hidden" name="entity" value="pembalap" id="pembalap-entity" data-field>', '<input type="hidden" name="entity" value="pembalap" id="pembalap-entity" data-field>', $template);
        }
        return $template;
    }
}
